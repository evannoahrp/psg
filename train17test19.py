...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def modeljst(self):
        try:
            ...
            # load dataset
            dataframeval = pd.read_csv("Data PSG 2019.csv")
            datasetval = dataframeval.values
            xtestval = datasetval[:,0:5]
            ytestval = datasetval[:,5:8]

            scaler_filename = "scaler.save"
            scaler = joblib.load(scaler_filename)
            testpreprocess = scaler.transform(xtestval)
            testdf = pd.DataFrame(testpreprocess, columns = dataframeval.columns[0:5])
            inputpreprocess = testdf.values
            # inputx = inputpreprocess[:, [0, 1, 4]]
            inputx = inputpreprocess

            outputYval = ytestval[:,kelas]
            yvalencoded = encoder.transform(outputYval)
            outputy = np_utils.to_categorical(yvalencoded)

            trainX, testX = input, inputx
            trainy, testy = output, outputy
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Klasifikasi PSG"))
        self.labelJudul1.setText(_translate("MainWindow", ""))
        self.labelJudul2.setText(_translate("MainWindow", ""))
